...
static uint32
insert(tst_db * db, uint32 node, const char *s, PyObject * value, int d, int len_of_s, uint32 parent)
{
	char c = s[d];
	uint32 x = node, no;
	if (x == 0) {
...
static uint32
search(tst_db * db, const char *key, int len_of_s)
{
...
static uint32
search_with_path(tst_db * db, const char *key, int len_of_s, char *path)
{
	int len_of_key = strlen(key);
	uint32 node = db->root;
	char c;
...
void
tst_put(tst_db * db, const char *key, PyObject * value)
{
	//printf("db: %x\n",db);
	int len_of_key = strlen(key);
	if (len_of_key <= 0)
		return;
	db->root = insert(db, db->root, key, value, 0, len_of_key, 0);
}
...
void
tst_delete(tst_db * db, const char *key)
{
	int len_of_key = strlen(key);
	uint32 node = db->root;
	char c;
...
static
    void
append_result(const char *key, PyObject* result, int *result_size)
{
	//printf("append: %s\n",key);
	//strcat(result, key);
	//strcat(result, "\r\n");
	PyList_Append(result,Py_BuildValue("s",key));
...
static
    void
dfs(tst_db * db, uint32 node, PyObject * result, int* result_size, char key_buf[], int d, int limit, enum SortingOrder sorting_order)
{
	uint32 first, second;
	//printf("--dfs--\n");
	if (node == 0)
...
static
    void
dfs_less(tst_db * db, uint32 node, const char *key, PyObject* result , int* result_size, char key_buf[], int d, int limit)
{
	char c;
	if (node == 0)
		return;
...
static
    void
dfs_greater(tst_db * db, uint32 node, const char *key, PyObject * result , int* result_size, char key_buf[], int d, int limit)
{
	char c;
...
void
tst_less(tst_db * db, const char *key, PyObject * result, int limit)
{
	int len_of_key = strlen(key);
	int n_result_size;
	...
	*result_size = 0;
	...
	dfs_less(db, root, key, result,result_size, base_key, 0, limit);

}

void
tst_greater(tst_db * db, const char *key, PyObject * result , int limit)
{
	int len_of_key = strlen(key);
	int n_result_size;
	int *result_size = &n_result_size;
	...
	*result_size = 0;
	...
	dfs_greater(db, root, key, result, result_size, base_key, 0, limit);

}

void
tst_prefix(tst_db * db, const char *prefix, PyObject *result, int limit, enum SortingOrder sorting_order)
{
	...
	if (*result_size < limit)
		dfs(db, db->data[node].mid, result, result_size, base_key, strlen(base_key), limit, sorting_order);
...
PyObject *
tst_get(tst_db * db, const char *key)
{
	int len_of_key = strlen(key);
	if (len_of_key <= 0)
		return 0;
